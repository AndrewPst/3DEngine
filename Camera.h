...
public:
	...
	{
		auto vertexes = ObjectTransformer::updateGlobalMatrix(object) * getCameraMatrix(); //Переводим координаты в глобальные и преобразуем в пространство камеры
		...
		for (size_t i = 0; i < vertexes.getRowsCount(); i++) { //Делим каждую вершину на W (Нормализуем?)
			...
			t[2] /= t[3];
			if (t[3] < 0)
				t[2] = -abs(t[2]); //сохраняем отрицательное значение
			t[3] /= t[3];
		}
